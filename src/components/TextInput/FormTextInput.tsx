...
export const FormTextInput = ({
  ...
  const accessibilityLabelRequired = required ? `, Required` : '';
  const accessibilityInputValue =
    (!secureTextEntry && value && isIOS) || showPassword ? `, ${value ?? ''}` : '';
  return (
    <A11yContainer>
      <A11yElement order={1}>
        <View
          ...
          {label ? (
            <Text
              style={[
                ...
                error?.error && {color: colors.fontDanger},
              ]}>
              {label}
              {required && (
                <Text
                  style={[styles.required, {color: colors.fontSecondaryInfo}]}>{`Required`}</Text>
              )}
            </Text>
          ) : null}
          ...
          {error && error.reason ? (
            <Text style={[styles.error, {color: colors.fontDanger}]}>{error.reason}</Text>
          ) : null}
        </View>
      </A11yElement>
    </A11yContainer>
